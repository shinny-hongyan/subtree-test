  ...
  mounted(){
    ...
    this.$tqsdk.on("rtn_data", function(){
      ...
      if (chartFoucs && !chartFoucs.more_data && chartFoucs.left_id && chartFoucs.right_id) {
        ...
        let chartHasMoreData = self.$tqsdk.get_by_path(['charts', CHART_ID, 'more_data'])
        if (!chartHasMoreData && self.$tqsdk.is_changed(klines)) {
          chartInstance.draw()
        }
      }
    })
  ...
  methods: {
    updateTqChart () {
      ...
      if (chartInstance) {
        ...
        chartInstance.setMainSeries(this.instrumentId, this.duration, klines)
